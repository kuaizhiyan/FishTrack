...
'''
使用方法：
添加了 --category-file 参数
若 CLI 给出 --text-promt 参数，则使用输入的文本
若 CLI 没有 --text-prompt 和 --category-file 则默认读取当前目录下的的 categories.txt 文档
输入 --category-file 参数给出指定分类文档存放路径
...
def parse_args():
    ...
    parser.add_argument('--category-file',type=str, default="categories.txt", help="本地类别文件路径（当未提供 --text-prompt 时使用）")
    ...
    for j, tok_list in enumerate(tokens_positive):
        for (beg, end) in tok_list:
            ...
            if beg_pos is None or end_pos is None:
                print(f"Warning: beg_pos or end_pos is None for label {labels[j]}")
                print(f"beg: {beg}, end: {end}, tokenized text: {tokenized}")
...
def read_categories(file_path):
    """读取类别文件，并将类别合并为一个以 '.' 分割的字符串"""
    if not os.path.exists(file_path):
        raise FileNotFoundError(f"类别文件 {file_path} 不存在！")
...
def main():
    ...
    if args.text_prompt is not None:
        ...
        text_prompt = read_categories(args.category_file)
    text_prompt = text_prompt.lower()
    text_prompt = text_prompt.strip()
    
    if not text_prompt.endswith('.'):
        text_prompt = text_prompt + '.'
    print('分类文本:',text_prompt)
    ...
    label_embeddings = torch.tensor(label_model.encode(label_name))
    embedding_dict = { i : label_embeddings[i] for i in range(len(label_embeddings))}
    # 清理显存
    del label_model
    ...
    for frame_id, img in enumerate(imgs):
        ...
        if isinstance(img, str):
            ...
            if 'GroundingDINO' in args.det_config:
                image_new = Image.open(image_path).convert('RGB')       # （1180，720）
                image_new = apply_exif_orientation(image_new)           # PIL.Image (1280,720) WxH
            else:
        ...
        with torch.no_grad():
            pred_track_instances = tracker.track(
                                                ...
                                                label_embedding_dict=embedding_dict,
                                                # rescale=False
                                                )
