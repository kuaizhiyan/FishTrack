...
class PAMSTransformer(BaseModule):
    ...
    Args:
        embed_dims (int): The embedding dimension size. Default is 256.
        decoder (OptConfigType): The configuration for the decoder module.
            If None, no decoder configuration is provided.
        positional_encoding (OptConfigType): Configuration for positional encoding.
            Typically includes settings for embedding dimensions.
        num_queries (int): The number of queries for the decoder. Default is 129.
        channel_mapper (OptConfigType): Configuration for channel mapping.
            If None, `feature_layers` is set to 1. Otherwise, it's set to the length
            of `in_channels` specified in `channel_mapper`.
        encoder (OptConfigType): The configuration for the encoder module.
            If None, the encoder is not initialized.
        train_cfg (OptConfigType): Training configuration, including options for loss
            computation, optimizer settings, or training strategies.
        test_cfg (OptConfigType): Testing configuration, including options for inference
    ...
    def __init__(self,
        ...
        if channel_mapper is None:
            ...
            self.feature_layers = len(channel_mapper['in_channels'])
    ...
    def pre_transformer(
        ...
        if self.feature_layers==1 :
            ...
            for i,feat in enumerate(img_feats):
                ...
                masks = None
                # [batch_size, embed_dim, h, w]
                pos_embed = self.positional_encoding[0](masks, input=feat) # feat[2,256,32,16]->[2,256,32,16];feat[2,256,16,8]->[2,256,16,8]
...
class PMNet(BaseModule):
    ...
    def __init__(self, 
        ...
        super(PMNet,self).__init__(init_cfg)
        self.num_queries = num_queries
        # self.encoder = DetrTransformerEncoder(**encoder)
        self.mspctrans = PAMSTransformer(    
    ...
    def forward(self,x):    
        ...
        # fetch out the first global query 
        global_query = (out['hidden_states'][-1][:,0],) 
        # return global_query as the representation
        return global_query
